...
\pdna{} variants achieve up to a 4.62 percentage point advantage over the baseline on
multi-gap sMNIST ($p < 0.05$ for self-attend vs.\ baseline), while a noise control
(random perturbation of equal magnitude) provides no benefit, confirming that the
advantage is structural rather than merely dynamic. These results provide evidence
...
\begin{figure}[h]
\centering
\includegraphics[width=0.8\textwidth]{figures/frequency_spectrum.png}
\caption{Distribution of learned oscillation frequencies $\omega$ across hidden
dimensions for Variant C (Pulse) and Variant E (Full \pdna{}).}
