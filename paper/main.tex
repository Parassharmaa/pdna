...
psMNIST results marked with $\dagger$ are partial (2 seeds for noise, others in progress).}
\label{tab:accuracy}
\begin{tabular}{lcc}
\toprule
\textbf{Variant} & \textbf{sMNIST} & \textbf{psMNIST} \\
\midrule
A. Baseline \cfc{} & 97.82 $\pm$ 0.12 & 82.45 $\pm$ 1.85 \\
B. \cfc{} + Noise & 97.78 $\pm$ 0.20 & 85.25 $\pm$ 0.73$^\dagger$ \\
...
On sMNIST, all variants achieve similar clean accuracy ($\sim$98\%), with the pulse variant
...
under gap conditions. Preliminary psMNIST results show CfC achieving 82.45\% on the
784-step pixel-by-pixel task.
...
($\sim$28--30\%), suggesting a fundamental information loss threshold. The multi-gap
...
\paragraph{Information density and gap sensitivity.}
Preliminary psMNIST results reveal a striking contrast: all variants show \emph{near-zero}
degradation ($-0.08 \pm 0.08\%$) across all gap levels. This is because psMNIST distributes
information sparsely---each of 784 timesteps carries only 1 pixel value---so missing a
contiguous block of timesteps loses proportionally little information. In contrast, sMNIST
packs 28 pixel values per timestep, making each timestep much more information-dense and
gaps correspondingly more damaging. This suggests that the Gapped evaluation protocol is
most informative for tasks with \emph{high information density per timestep}, where the
pulse mechanism's ability to maintain coherent state during gaps becomes critical
(Figure~\ref{fig:gap_sensitivity}).

\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{figures/gap_sensitivity_comparison.png}
\caption{Gap sensitivity depends on information density. (a) sMNIST (28
features/timestep): severe degradation under gaps, with pulse-augmented variants
showing clear advantages. (b) psMNIST (1 feature/timestep): near-zero degradation
regardless of variant, because missing a few pixels out of 784 loses little information.}
...
initial value of 0.01 to $0.663 \pm 0.032$ (Variant C) and $0.649 \pm 0.026$
(Variant E), a $\sim$66$\times$ increase. This confirms the model discovers significant
utility in the oscillatory signal during training.
\item \textbf{Frequency diversity}: The learned frequency parameters $\omega$ span
the range $[0.06, 10.02]$ with mean $2.17$, indicating the model discovers multiple
useful oscillation frequencies rather than collapsing to a single dominant frequency.
The broad distribution suggests different hidden dimensions specialize in different
temporal scales.
...
\includegraphics[width=\textwidth]{figures/frequency_spectrum.png}
\caption{Learned frequency distribution $\omega$.}
\label{fig:freq_spectrum}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.48\textwidth}
\includegraphics[width=\textwidth]{figures/alpha_growth.png}
...
magnitude, indicating diverse temporal scales. (b) The pulse strength parameter $\alpha$
grows $\sim$66$\times$ from initialization, confirming the model actively discovers
utility in the oscillatory signal.}
...
\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{figures/training_and_multigap.png}
...
Our current evaluation is based on sMNIST, a relatively short-sequence task (28
timesteps). While we include psMNIST (784 steps) and sCIFAR-10 (1024 steps) in our
experimental design, these longer-sequence results are still being collected at the time
of writing. The statistical significance of our results is limited by the small number
of seeds ($n = 5$), though large effect sizes (Cohen's $d > 1.0$) suggest the effects
are meaningful. The \cfc{} backbone processes sequences in parallel, which means our
``pulse'' operates as a post-hoc augmentation rather than a true continuous-time internal
dynamic. A sequential architecture (e.g., using \texttt{torchdiffeq}) would allow the
pulse to genuinely evolve state between input steps, potentially yielding stronger
results at the cost of GPU parallelism.
...
sequences: on sMNIST, the pulse variant achieves 92.86\% multi-gap accuracy vs.\
