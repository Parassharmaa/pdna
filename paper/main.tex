...
Our primary evaluation is based on sMNIST (28 timesteps). Preliminary psMNIST (784
steps) results suggest that gap sensitivity depends on information density per timestep,
but sCIFAR-10 (1024 steps) results are still being collected. Evaluation on tasks from
the Long Range Arena~\citep{tay2021long} would strengthen generalizability claims.
...
between input steps, (2) extending to tasks from the Long Range
Arena~\citep{tay2021long} and real-world datasets with natural temporal gaps (medical
monitoring, autonomous driving sensor data), (3) exploring the learned frequency spectrum
as a form of unsupervised temporal representation learning, and (4) applying gap training
(training \emph{with} gaps) to combine architectural robustness with data augmentation.
