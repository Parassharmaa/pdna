...
\node[op, below=0.7cm of cfc] (add1) {$+$};

\node[op, below=0.7cm of add1] (add2) {$+$};
...
\node[modbox=green!70!black, left=1.8cm of add1]
...
\draw[arr, green!50!black, densely dashed]
    ($(cfc.south)!0.5!(add1.north)$) -- ++(-2.7,0) |- (pulse.west);
\node[lab] at ($(cfc.south)!0.5!(add1.north)+(-1.35,-0.15)$) {$h_\text{cfc},\; t$};

% === Self-Attend Module (right of add2) ===
\node[modbox=purple, right=1.8cm of add2]
    (sa) {\textbf{Self-Attend Module}\\[2pt]
    $\beta \!\cdot\! W_\text{self} \!\cdot\! \sigma(h)$};

\draw[arr, purple!60] (sa) -- node[above, lab] {$\beta \!\cdot\! s$} (add2);
\draw[arr, purple!60, densely dashed]
    ($(add1.south)!0.5!(add2.north)$) -- ++(2.7,0) |- (sa.east);
\node[lab] at ($(add1.south)!0.5!(add2.north)+(1.35,-0.15)$) {$h_\text{pulse}$};
