...
\textit{(Results will be populated from the extended experiment run.)}

\subsection{Gap Robustness}

\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{../reports/figures/degradation_curves.png}
\caption{Accuracy under increasing gap severity. \pdna{} variants degrade less
than baseline when input is interrupted. The noise control (B) degrades more than
baseline, confirming that \emph{structured} oscillation specifically helps.}
...
\begin{table}[h]
\centering
\caption{Degradation (Gap 0\% $-$ Gap 30\% accuracy). Lower = more robust. Bold indicates least degradation per task.}
